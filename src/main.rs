...
use crate::visualize::log_given_freqs;
use audio_file::read_audio_file;
use audio_process::{fft_samples, get_fundamental_frequency};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..1 {
        ...
        // 3. Calculate note at each moment
        let main_freq_per_t: Vec<f64> = left_chan_results
            .freqs
            .iter()
            .map(|freqs| get_fundamental_frequency(freqs, left_chan_results.hz_per_element))
            .collect();
        log_given_freqs(
            &rec,
            &title,
